...
with tempfile.TemporaryDirectory(dir=output.parent) as directory:
    with duckdb.connect(config=dict(memory_limit='4GB',threads=4,temp_directory=directory,max_temp_directory_size='8GB')) as c:
        ...
        build_examples_v2(c,config['gap_seconds'],datetime.fromisoformat(config['validation_start_utc']),datetime.fromisoformat(config['test_start_utc']))
        ...
        invalid = c.execute('SELECT count(*) FROM saved WHERE (label IS NULL)!=(exclusion_reason IS NOT NULL) OR event_count_30d<1 OR event_count_24h>event_count_7d OR event_count_7d>event_count_30d OR alarm_count_24h>alarm_count_7d OR alarm_count_7d>alarm_count_30d OR alarm_count_30d>event_count_30d OR last_event_age_hours<0 OR last_event_age_hours>=720 OR active_days_30d NOT BETWEEN 1 AND 30 OR log_gap_hours_30d NOT BETWEEN 0 AND 720 OR label NOT IN (0,1)').fetchone()[0]
        if invalid:
...
print(json.dumps(dict(config=config,dataset_version='v2',snapshot_policy='channel_has_record_in_previous_30_days',feature_windows=['24h','7d','30d'],output_bytes=output.stat().st_size,groups=summary,training=False,inference=False),default=str,indent=2))
