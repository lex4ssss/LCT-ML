...
Каждый ответ /predict и каждая строка /risk_map несут `alert`, `model_threshold` и `horizon_hours`, по ним backend считает уровень и SLA. `model_threshold` дан в единицах `probability`: у датчиков это рабочий порог 0,147, у объектов откалиброванная вероятность на пороге сырой оценки (0,652 для инцидентов, 0,559 для отказов). Решает поле `alert`. Калибровка ступенчатая, поэтому на ступеньке порога `probability ≥ model_threshold` иногда расходится с `alert`: 1 объект из 77 на демо-часах.

## Прогноз отказа насоса (синтетика)

Флаг `--pump-decision outputs/ml-pumps/run-004/pump-decision.json` включает POST /predict_pump. Тело: `pump_id` (NPV_2_1…NPV_2_4) и `readings`, от 1 до 97 показаний 12 датчиков с шагом 5 минут, старшее первым. Ответ: два прогноза, на 0,5 и 1 ч, в каждом `probability`, `alert`, `model_threshold`, `horizon_hours`, плюс `synthetic: true`. Если показаний меньше 97, недостающая история заполняется нормальным профилем насоса. Модель обучена на синтетических данных, к объектам Москоллектора она не относится.
